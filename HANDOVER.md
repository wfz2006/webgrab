...
## ✅ 抖音支持：真机跑通了（2026-08-04 登录态实测）

接着上一节的更正继续查，最终定位并修好了真正的问题。**"点开视频 → 资源列表出现可下载的主视频"这条链路，现在在真机上跑通了。**

### 真正的问题：时序假设错了，而且错得很离谱

给探针加计数器实测拿到的关键数据：

```
reportPayload 触发时刻       t = 10.3s   ← module/feed 响应到达（页面初始加载）
响应到达时 URL 上的 modal_id  (空)
响应里的 aweme_list          10 条，其中就包含用户后来点开的那条
history.replaceState 补 modal_id  t = 49.3s  ← 用户点击
点击后新发的 module/feed 请求     0 次
```

也就是说：

- **整屏信息流（约 10 条，每条都带完整播放地址）在页面刚加载时就一次性取回来了**；
- 响应到达时，用户还没决定看哪条，URL 上自然没有 `modal_id`；
- 用户几十秒后点开某条时，抖音**不再发任何请求**，只是 `history.replaceState` 把 `modal_id` 补进 URL。

而之前的实现是在响应到达那一刻做"要么按 `modal_id` 匹配、要么丢弃"——那个时刻根本还不知道用户要看哪条，**必然走丢弃分支**。这不是几毫秒的竞态，是相差几十秒的设计错误。

### 修法：先缓存，等用户选定再发布

`content/douyin-probe.js` 改为：

- `module/feed` 的 `aweme_list` 到达时，把每条都缓存进 `Map<aweme_id, detail>`（上限 200 条，按插入序淘汰最早的，防止无限滚动把内存撑大）；此时**不发布任何资源**；
- 包装 `history.pushState` / `replaceState` 并监听 `popstate`/`hashchange`，URL 一变就按当前 `modal_id` 去缓存里查，命中就发布那一条。**必须包装 history —— 抖音用的是 `replaceState`，只听 `popstate` 收不到**；
- `aweme/detail` 这类单条接口仍然立即发布（分享链接、作品页等入口是明确的"就是这一条"，不依赖 `modal_id`）。

包装 history 是链式的（先调原函数、再做自己的事），不改变站点自身行为。
...
| 项 | 值 |
|---|---|
| mediaId | `7666072975716816561`（与 URL 上的 modal_id 一致） |
| 标题 | 《悬案》第二单元，卞记旅馆灭门案！ |
| 分辨率 / 时长 | 2560×1440 / 12:53 |
| 大小 | 71,025,569 字节 |
| 备用地址 | 2 个 |

对捕获到的地址发 Range 请求验证**确实可下载**：

```
status: 206
content-range: bytes 0-63/71025569   ← 总大小与上报值完全一致
文件头: 00 00 00 20 66 74 79 70 69 73 6f 6d  → "ftypisom"，合法 MP4
```

换一条视频（《灰骑士的一生》，`7643099774119365934`，1920×1080）同样正确捕获，可重复生效。

### 测试

`tests/douyin-probe.test.mjs` 7 → **11** 条。新增 4 条，其中 3 条经"回退到旧探针即失败"验证：

- 首屏 `module/feed` 到达时没有 `modal_id`，用户之后点开某条时才发布缓存里对应的那条
- 点开的作品不在缓存里时不产出资源，且不影响之后点开缓存中的作品
- 同一条作品上反复触发 URL 变化只发布一次
- 缓存有上限，超出后最早的条目被淘汰（内存有界的证明）

测试上下文补了 `history` / `addEventListener` 桩，用 `openModal(id)` 模拟"用户点开某条 → 页面 replaceState 补 modal_id"。
